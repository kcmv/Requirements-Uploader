...
export class AnswersService {
  ...
  async create(_createAnswerDto: CreateAnswerDto): Promise<Answer> {
    ...
    const purpose_field_type = await this.purposeFieldTypeService.findOne(
      _createAnswerDto.purpose_field_type
    );
  ...
  async update(id: string, _updateAnswerDto: UpdateAnswerDto): Promise<Answer> {
    const answer: any = await this.findOne(id);

    const purpose_field_type: any = await this.purposeFieldTypeService.findOne(
      _updateAnswerDto.purpose_field_type || 0
    );
